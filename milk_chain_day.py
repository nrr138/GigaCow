...
def milk_chain(farm,type,farmname,Bool):
    ...
    for i in range(len(kor)):
        #calculates the total milk per day
        if kor[i] not in hold2:
            hold2.append(kor[i]) 

        hold.append(int(kor[i]))
        
        if abs(star_time[i]-star_time[0]) > 1440*day:
            for i in hold2:
                if i not in cow_dict4:
                    cow_dict4[i]=1
                else:
                    cow_dict4[i]+=1
            hold2=[]
    ...
    if Bool:
        plt.title("Average milk produced per day at farm " + farmname + " for robot " + type)
    else:
        plt.title("Average milk produced per day at farm " + farmname)
...
milk_chain('RobotMilkings_A6.csv','VMS 1', 'f454e660',False)